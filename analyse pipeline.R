...
sce = readRDS("Sertoli7to150_2markers.rds")
snames = sce$Sample
...
df.in=read.table("Sert.ENS2Name.tsv", header = FALSE, sep='\t')
dim(df.in)
to.convert = df.in$V1
gene.names = df.in$V2
...
plotExpression(sce, features=c("CLU","AR","GATA4", "AMH",
                                         "FSHR", "SOX9","WT1","VIM"), x="Sample", colour_by="Sample")



plotExpression(sce, features=c("CDH1", "CDH2","CDH3", "GJA1",
...
plotExpression(sce, features=sox, x="Sample", colour_by="Sample")
...
plotExpression(sce, 
               features=c("NR4A1","CEBPB","SP1" ), x="Sample", colour_by="Sample")




##
plotExpression(sce.corrected, features=c("CLU","AR","GATA4", "AMH",
                                         "FSHR", "SOX9","WT1","VIM"), x="Sample", colour_by="Sample")
